...
public class SparkSQLUtil extends PhoenixUtil {
	...
	public static <E, T> String createGetSQL(QueryParam<T> param) throws Exception {
		...
		params.add(String.join(" AND ", conditions).toUpperCase());
		params.add(getGroupByCondition(distinctFieldNames).toUpperCase());
		params.add(getOrderByCondition(param).toUpperCase());
