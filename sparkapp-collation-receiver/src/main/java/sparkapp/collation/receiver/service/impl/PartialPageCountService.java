...
public class PartialPageCountService {
	...
	public List<PartialPageCount> getAll(String url) throws Exception{
		...
		if(urlTaggedTable.count() > 0) {
			...
			joinTable = joinTable.filter(col("NAME").equalTo(MetricTypeEnum.PAGE_COUNT.name()))
			.select(urlTaggedTable.col("URL"), urlTaggedTable.col("TKEY"), urlTaggedTable.col("TVALUES"), metricTable.col("NAME"));
			joinTable.show();
