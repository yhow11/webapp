...
/**
 * @ngdoc function
 * @name fingerPrintApp.controller:AddNewMetricController
 * @description # AddNewMetricController Controller of the fingerPrintApp
 */
angular.module('fingerPrintApp').controller(
		'AddNewMetricController',
		function($rootScope, $scope, $q,  $timeout, $stateParams, metricService, metricTypeService, keyService) {
			...
			$scope.metrics = {
					...
					keyfield: {
						...
						search: function(query) {
							var deferred = $q.defer();
							keyService.getAll("%25"+query+"%25","","").then(function(data){
								if(data.data){
									deferred.resolve( data.data.data );
								}
							});
