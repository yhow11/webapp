...
(function(window) {
	...
	function define_clientutil() {
		...
		CLIENTUTIL.send = function(visitorID, fingerprintdata, type, title, url) {
			...
			xhttp.send("log=" + data.lead_id + "||" + data.browserFP + "||"
					+ data.deviceFP + "||" + data.timestamp + "||" + type
					+ "||" + url + "||" + "N/A" + "||" + data.sessionID + "||"
					+ (data.elapsedtime || 0) + "||"
					+ (data.country || "unknown") + "||" + visitorID);
	...
	function define_fingerPrint(COOKIEUTIL, CLIENTUTIL, ELEMENTUTIL, UUID,
			TIMERUTIL) {
		var FingerPrint = {};
		FingerPrint.getDeviceFP = function(client) {
			...
			var deviceDataPoints = screenPrint + OS + CPU + device + timeZone
					+ language + fonts;
		...
		FingerPrint.getBrowserFP = function(client) {
			...
			var browserDataPoints = browser_browser + browser_browser_version
					+ browser_engine + browser_engine_version
					+ browser_localstorage + browser_session_storage
					+ browser_plugins + browser_canvas_print
					+ browser_mime_type + browser_is_mime_type
					+ browser_system_language + user_agent;

			return CryptoJS.MD5(browserDataPoints)
		...
		FingerPrint.getData = function() {
			var data = typeof window.localStorage.fingerPrintData !== "undefined" ? JSON
					.parse(window.localStorage.fingerPrintData)
					: {};
		...
		FingerPrint.sendClickedEvent = function(e) {
			...
			if (visitorID) {
				...
				var title = e.target.title != '' ? e.target.title
						: e.target.innerText;
				;
				CLIENTUTIL.send(visitorID, FingerPrint.getData(), "hover",
						title, href);
			}
		};
		...
		FingerPrint.sendLeavedEvent = function() {
			...
			if (visitorID) {
				...
				CLIENTUTIL.send(visitorID, data, "leaved", document.title,
		...
		FingerPrint.sendVisitedEvent = function() {
			...
			if (typeof visitorID !== 'undefined') {
				TIMERUTIL.getTime(baseURL).then(
						function(data) {
							TIMERUTIL.currentime = data.trim();
							COOKIEUTIL.write("starttime", data);
							CLIENTUTIL.send(visitorID, FingerPrint.getData(),
				...
				xhttp.open("GET", baseURL + 'segment/getByVisitor?visitorID='
						+ visitorID, true);
				xhttp.send();
			}
		...
		FingerPrint.getVisitor = function(baseURL, data) {
			return new Promise(function(resolve, reject) {
				...
				if (visitorID) {
					...
					xhttp.open("GET", baseURL + 'visitor/get?sessionID='
							+ data.sessionID + "&browserFP=" + data.browserFP
							+ "&deviceFP=" + data.deviceFP, true);
					xhttp.send();
				}
		...
		FingerPrint.init = function(baseURL) {
			...
			$.getJSON('//ipinfo.io/', function(data) {
				...
				var fingerPrintData = FingerPrint.getData();
				fingerPrintData.country = data.country + "-" + data.city
				FingerPrint.setData(fingerPrintData);
			});
			...
			FingerPrint.getVisitor(baseURL, FingerPrint.getData()).then(
					function(data) {
	...
	if (typeof (FingerPrint) === 'undefined') {
		window.FingerPrint = define_fingerPrint(define_cookieutil(),
				define_clientutil(), define_elementutil(), define_uuid(),
				define_timerutil(define_cookieutil()));
	} else {
